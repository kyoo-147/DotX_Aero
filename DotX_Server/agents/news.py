#!/usr/bin/python3
"""
Tính năng tin tức, dành cho bot - Gửi thông báo tin tức đến máy khách
...
# Thiết lập cấu hình
api_key = os.environ.get("OPENAI_API_KEY", "open_api_key")                  # Bắt buộc, sử dụng chuỗi không có thật Llama.cpp
api_base = os.environ.get("OPENAI_API_BASE", "http://localhost:8000/v1")    # Bắt buộc, hãy sử dụng https://api.openai.com cho OpenAI

agentname = os.environ.get("AGENT_NAME", "Newsbot")                         # Thiết lập tên cho chat bot
mymodel = os.environ.get("LLM_MODEL", "models/7B/gguf-model.bin")           # Chọn mô hình để sử dụng, ví dụ: gpt-3.5-turbo cho OpenAI

DEBUG = os.environ.get("DEBUG", "false").lower() == "true"                  # Đặt thành True để bật chế độ gỡ lỗi

MAXTOKENS = int(os.environ.get("MAXTOKENS", 16*1024))                       # Số lượng mã thông báo tối đa để gửi tới LLM
TEMPERATURE = float(os.environ.get("TEMPERATURE", 0.0))                     # Nhiệt độ LLM

USE_SYSTEM = os.environ.get("USE_SYSTEM", "false").lower == "true"          # Sử dụng hệ thống trong lời nhắc trò chuyện nếu Đúng

RESULTS = int(os.environ.get("RESULTS", 10))                                # Số kết quả trả về từ Weaviate
ALPHA_KEY = os.environ.get("ALPHA_KEY", "alpha-key")                        # Khóa API Alpha Vantage

COMPANY = os.environ.get("COMPANY", "Google")                               # Công ty sử dụng cho tin tức chứng khoán

CITY = os.environ.get("CITY", "Los Angeles")                                # Thành phố để sử dụng cho tin tức thời tiết  

# Mặc định nhắc nhở
prompts = {
    "greeting": "Hello",
    "agentname": "Aero",
    "baseprompt": "You are {agentname} AI, an extremely intelligent support assistant, you have a very friendly and polite personality. The current date is {date}.\n\nYou should give concise responses to very simple questions, but provide thorough responses to more complex and open-ended questions. Help users answer questions completely and accurately.",
...
# Test OpenAI API
while True:
    log("Test with OpenAI API...")
    try:
        log(f"Using OpenAI library version {openai.__version__}")
        log(f"Connecting to OpenAI API at {api_base} using specify model {mymodel}")
        ...
        if len(models.data) == 0:
            log("*****LLM: No available models found - try checking your API key and endpoint*****")
        ...
        log("OpenAI API error found: %s" % e)
...
if __name__ == "__main__":
    print("Rabbit AI News Bot")
    print("******************")

    # Date
    current_date = datetime.datetime.now()
    print(f"Current date at your location:: {current_date.strftime('%B %-d, %Y')}")
    print("******************")
    # Initialize context
    context = base_prompt()
    ...
    print(f"Weather at your location:\n{weather}")
    print("******************")

    # Query LLM for stock
    stock = handle_stock_command(COMPANY)
    print(f"{COMPANY} Shares of the company you want to refer to: {stock}")
    print("******************")
